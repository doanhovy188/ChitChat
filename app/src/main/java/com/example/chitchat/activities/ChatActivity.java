        ...
        database.getReference().child("chatRooms").child(senderRoom).child("messages")
                .addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        Date date = new Date();
                        if (messages.size() > 0 && messages.get(messages.size() - 1).getSenderId() != null && !messages.get(messages.size() - 1).getSenderId().equals(senderID) && messages.get(messages.size() - 1).getTimesent() - date.getTime() < 10000) {
